...
<html>
    <head>
        ...
        <!-- App title -->
        <title>Phylo Results - PredHPI</title>
...
$namer = $_GET['result'];
$filenameInfo = 'tmp/' . $namer . '_PhyloPredTaskInfo.txt';

$running_bool = file_exists('tmp/' . $namer . '_PhyloPredTaskInfo.txt'); ///if exist is running
$finished_bool = file_exists('tmp/' . $namer . '_PhyloPredTabularInfo.txt'); ///if exist is finished
...
if($finished_bool){
    ...
    $num_interactions = sizeof(file('tmp/' . $namer . '_PhyloPredTabularInfo.txt'));
    ...
    if(!($empty_results)){
        ...
        $downloadFile = "tmp/" . $namer . "_PhyloPredTabularInfo.txt";echo '<a href="'.$downloadFile.'" class="btn btn-success waves-effect waves-light"download>Full Results <i class="fa fa-download" style="font-size: 20px"></i></a>'; 
        ...
        while(! feof($fileInfo)){
            ...
            if($line != false && $line != '\n' && trim($line) != ''){
                ...
                if(trim($separatedLine[0])=="Host" ||trim($separatedLine[0])=="Pathogen"){
                    echo "<b>".$separatedLine[0].":</b>";
                } else {
                    if($odd){
 ...
 <!--                           <table id="datatable-buttons" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                <thead>
                                    <tr role="row">
                                        ...
                                        <th class="sorting_asc" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Host: activate to sort column descending" style="width: 169px;">Host</th>
                                        <th class="sorting" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" aria-label="Pathogen: activate to sort column ascending" style="width: 248px;">Pathogen</th>
...
if($finished_bool && !($empty_results)){
    ...
    if($num_interactions<=10000){
        ...
        echo '                          <thead>
                                        <tr role="row">
                                            ...
                                            <th class="sorting" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" aria-label="Similarity of Patterns: activate to sort column ascending" style="width: 248px;">Similarity</th>
                                        </tr>
                                    </thead>';
    ...
    echo '<h1 style="color: #039cfd;">FINISHED.</h1><h2 style="color: #1bb99a;">No successfull Host-Pathogen Interactions were predicted using Phylogenetic Profiling method on this dataset, please try another PredHPI module and resubmit. Thanks.</h2>';
...
?>
        ...
        <script type="text/javascript">
            ...
            $("#seeNetwork").click(function() {
                var resultUrl = "http://bioinfo.usu.edu/PredHPI/network-visualization.php?result=" + <?php  echo(json_encode($namer)); ?>+"&method=phyloPred";
