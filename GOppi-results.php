...
<html>
    <head>
        ...
        <!-- App title -->
        <title>GOPred Results - PredHPI</title>
...
$namer = $_GET['result'];
$filenameInfo = 'tmp/' . $namer . '_GOppiPredTaskInfo.txt';

$running_bool = file_exists('tmp/' . $namer . '_GOppiPredTaskInfo.txt'); ///if exist is running
$finished_bool = file_exists('tmp/' . $namer . '_GOppiPredTabularInfo.txt'); ///if exist is finished
...
if($finished_bool){
    ...
    $fileTabular = fopen('tmp/' . $namer . '_GOppiPredTabularInfo.txt',"r");
    ...
    if(!($empty_results)){
        ...
        $downloadFile = "tmp/" . $namer . "_GOppiPredTabularInfo.txt";echo '<a href="'.$downloadFile.'" class="btn btn-success waves-effect waves-light"download>Full Results <i class="fa fa-download" style="font-size: 20px"></i></a>'; 
...
if($finished_bool && !($empty_results)){
    ...
    if($num_interactions<=10000){
        ...
        echo '                          <thead>
                                        <tr role="row">
                                            <th class="sorting_asc" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Interaction: activate to sort column descending" style="width: 169px;">Interaction</th>
                                            ...
                                            <th class="sorting" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" aria-label="Similarity: activate to sort column ascending" style="width: 248px;">Similarity</th>
                                            <th class="sorting" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" aria-label="Host_GO_Terms: activate to sort column ascending" style="width: 121px;">Host_GO_Terms</th>
                                            <th class="sorting" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" aria-label="Pathogen_GO_Terms: activate to sort column ascending" style="width: 121px;">Pathogen_GO_Terms</th>
                                        </tr>
                                    </thead>';
    ...
    while(! feof($fileTabular)){
        ...
        if($line != false && $line != '\n' && trim($line) != ''){
            ...
            if($separatedLine[0] == "YES"){
                ...
                $hostTerms = $separatedLine[5];
                $pathogenTerms = $separatedLine[5];

                if(substr_count($hostTerms,",")>5){
                    $hostTermsMod = substr($hostTerms,0,65);
                    $hostTermsMod .= "</br>";
                    $hostTermsMod .= substr($hostTerms,65,strlen($hostTerms));
                    $hostTerms = $hostTermsMod;
                }

                if(substr_count($pathogenTerms,",")>5){
                    $pathogenTermsMod = substr($pathogenTerms,0,65);
                    $pathogenTermsMod .= "</br>";
                    $pathogenTermsMod .= substr($pathogenTerms,65,strlen($pathogenTerms));
                    $pathogenTerms = $pathogenTermsMod;
                }
                ...
                echo '<td>' . $hostTerms . '</td>';
                echo '<td>' . $pathogenTerms . '</td>';
                echo '</tr>';
            }
    ...
    echo '<h1 style="color: #039cfd;">FINISHED.</h1><h2 style="color: #1bb99a;">No successfull Host-Pathogen Interactions were predicted using GOSim method on this dataset, please try another PredHPI module and resubmit. Thanks.</h2>';
...
?>
        ...
        <script type="text/javascript">
            ...
            $("#seeNetwork").click(function() {
                var resultUrl = "http://bioinfo.usu.edu/PredHPI/network-visualization.php?result=" + <?php  echo(json_encode($namer)); ?>+"&method=GOPred";
