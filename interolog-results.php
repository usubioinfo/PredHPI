...
<html>
    <head>
        ...
        <!-- App title -->
        <title>Interolog Results - PredHPI</title>
...
$running_bool = file_exists('tmp/' . $namer . '_InterologTaskInfo.txt'); ///if exist is running
$finished_bool = file_exists('tmp/' . $namer . '_InterologTabularInfo.txt'); ///if exist is finished
...
if($finished_bool){
    ...
    $num_interactions = sizeof(file('tmp/' . $namer . '_InterologTabularInfo.txt'));
    ...
    if(!($empty_results)){
        ...
        $downloadFile = "tmp/" . $namer . "_InterologTabularInfo.txt";echo '<a id="downloadFileBtn" href="'.$downloadFile.'" class="btn btn-success waves-effect waves-light"download>Full Results <i class="fa fa-download" style="font-size: 20px"></i></a>';
        ...
        $downloadFile = "tmp/" . $namer . ".json";echo ' <a id="downloadNetBtn" href="'.$downloadFile.'" target="_blank" class="btn btn-purple waves-effect waves-light"download> Download Network <i class="fa fa-download" style="font-size: 20px"></i></a>';
        echo '<p>';

        $filenameInfo = 'tmp/' . $namer . '_InterologTaskInfo.txt';
...
if($finished_bool && !($empty_results)){
    ...
    if($num_interactions<=10000){
        ...
        echo '                          <thead>
                                        <tr role="row">
                                            ...
                                            <th class="sorting" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" aria-label="Homologous Proteins Interacting: activate to sort column ascending" style="width: 121px;">Homologous Proteins Interacting</th>
                                            <th class="sorting" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" aria-label="Protein-Protein Database: activate to sort column ascending" style="width: 121px;">Protein-Protein Database</th>
                                        </tr>
                                    </thead>';
        ...
        echo '                          <thead>
                                        <tr>
                                            <th>Host</th>
                                            <th>Pathogen</th>
                                            <th>Homologous Proteins Interacting</th>
                                            <th>Database</th>
                                        </tr>
                                    </thead>';
    ...
    echo '<h1 style="color: #039cfd;">FINISHED.</h1><h2 style="color: #1bb99a;">No successfull Host-Pathogen Interactions were predicted using Interolog method on this dataset, please try another PredHPI module and resubmit. Thanks.</h2>';
...
?>
        ...
        <script type="text/javascript">
            ...
            $("#seeNetwork").click(function() {
                var resultUrl = "http://bioinfo.usu.edu/PredHPI/network-visualization.php?result=" + <?php  echo(json_encode($namer)); ?>+"&method=interolog";
